...
void Problem::printStats() {
    std::cout << boost::format("%1% (%2% Owned)\nRarity: %3%\nPrimary Colour/Cost: %4%/%5%\nSecondary Colour/Cost: %6%/%7%\nNeutral Cost: %8%\nBonus: %9%\nStarting: %10%") 
        % accessName() % accessFrequency() % rarityMap.find(accessRarity())->second % colourMap.find(primaryColour)->second % primaryColourConfrontCost 
        % colourMap.find(secondaryColour)->second % secondaryColourConfrontCost % neutralConfrontCost % pointBonus 
